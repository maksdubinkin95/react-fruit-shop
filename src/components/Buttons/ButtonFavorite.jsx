...
const ButtonFavorite = ({disabled, title, img, price, description, setIsLiked, isLiked, addToFavorites }) => {
    return (
        <button disabled={disabled} className="card__button button-like" onClick={() => setIsLiked(!isLiked)}>
            <div className="button-like--active" onClick={() => addToFavorites({ title, img, price, description })}>
